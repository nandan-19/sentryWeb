...
// WebSocket channel for realtime sandbox updates
const GO_SANDBOX_WS_URL = "ws://localhost:8081/ws";

// HTTP endpoint for triggering sandbox analysis
const GO_SANDBOX_API_URL = "http://localhost:8081/analyze";

let sandboxSocket: WebSocket | null = null;

export async function checkPayloadWithOllama(
    payload: string
...
export function initSandboxWebSocket() {
    ...
    sandboxSocket.onclose = () => {
        ...
        setTimeout(initSandboxWebSocket, 5000);
    };
...
/* ------------------------------------------------ */
/*  TRIGGER REMOTE SANDBOX                          */
/* ------------------------------------------------ */

export async function triggerRemoteSandbox(targetUrl: string) {
    try {
